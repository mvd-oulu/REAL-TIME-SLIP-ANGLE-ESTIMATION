...
void GGA_Handler() //Rec'd GGA
{
    ...
    else if (!useBNO08x && !useCMPS && !useDual) 
    {
        ...
        digitalWrite(GPSGREEN_LED, LOW);   //Make sure the Green LED is OFF
        itoa(65535, Heading, 10);       //65535 is max value to stop AgOpen using IMU in Panda
        BuildNmea();
    }
...
//        else  //No IMU so put dual Heading & Roll in direct.
        {
            ...
            dtostrf(heading, 4, 2, Heading); //this heading from GNSS. not from IMU
        }
    }
}
...
void VTG_Handler()
{
  // vtg heading //direction of speed, relative to the true north
  parser.getArg(0, vtgHeading); // need to convert to radian

  // vtg Speed knots
  parser.getArg(4, speedKnots);
  parser.getArg(6, speedkmh);
